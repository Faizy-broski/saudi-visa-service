...
function StatCard({ label, value, sub, icon, grad }: { label: string; value: string | number; sub: string; icon: React.ReactNode; grad: string }) {
  return (
    <div className="rounded-2xl p-6 bg-white flex items-start gap-4" style={{ border: '1px solid #eef2f7', boxShadow: '0 2px 16px rgba(10,56,90,0.05)' }}>
      <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 text-white" style={{ background: grad }}>
        {icon}
      </div>
      <div>
        <div className="text-2xl font-bold leading-none mb-1" style={{ color: '#0A385A' }}>{value}</div>
        <div className="text-sm font-semibold" style={{ color: '#0A385A' }}>{label}</div>
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Bar chart */}
        <div className="lg:col-span-2 rounded-2xl p-6 bg-white" style={{ border: '1px solid #eef2f7' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              return (
                <div key={key}>
                  <div className="flex justify-between text-sm mb-1.5">
                    ...
                    <span className="font-bold text-xs" style={{ color: '#0A385A' }}>{count} bookings</span>
                  </div>
                  <div className="h-2.5 rounded-full" style={{ background: '#eef2f7' }}>
                    <div className="h-2.5 rounded-full" style={{ width: `${pct}%`, background: 'linear-gradient(to right,#3CA5D4,#0E3254)' }} />
                  </div>
                </div>
              );
            })}
        ...
        {/* Quick actions */}
        <div className="rounded-2xl p-6 bg-white" style={{ border: '1px solid #eef2f7' }}>
          <h3 className="font-bold text-base mb-5" style={{ color: '#0A385A' }}>Quick Actions</h3>
          <div className="space-y-3">
            {[
              ...
              <Link
                key={label}
                href={href}
                className="flex items-center justify-between px-4 py-3 rounded-xl transition-all hover:bg-[#f8fbff]"
                style={{ border: '1px solid #eef2f7' }}
              >
                <div className="flex items-center gap-3">
                  <span className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#3CA5D4,#0E3254)', color: '#fff' }}>{icon}</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <svg viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth={2} className="w-4 h-4"><polyline points="9 18 15 12 9 6"/></svg>
                </div>
              </Link>
            ))}
          </div>
      ...
      <div className="rounded-2xl overflow-hidden bg-white" style={{ border: '1px solid #eef2f7' }}>
        <div className="px-6 py-4 flex items-center justify-between border-b" style={{ borderColor: '#eef2f7' }}>
          <h3 className="font-bold" style={{ color: '#0A385A' }}>Recent Bookings</h3>
          <Link href="/admin/bookings" className="text-xs font-bold px-4 py-2 rounded-lg" style={{ background: 'linear-gradient(to right,#3CA5D4,#0E3254)', color: '#fff' }}>View all →</Link>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {apps.map((a) => (
                <tr key={a.id} className="border-b hover:bg-[#f8fbff] transition-colors" style={{ borderColor: '#f3f4f6' }}>
                  <td className="px-5 py-3.5 font-mono text-xs font-bold" style={{ color: '#3CA5D4' }}>{a.reference_number}</td>
                  ...
                  <td className="px-5 py-3.5 text-xs" style={{ color: '#6b7280' }}>{VISA_LABEL[a.visa_type] ?? a.visa_type}</td>
                  <td className="px-5 py-3.5 text-xs font-bold" style={{ color: '#da6d3f' }}>{a.amount_usd ? `$${Number(a.amount_usd).toFixed(2)}` : '—'}</td>
                  <td className="px-5 py-3.5 text-xs" style={{ color: '#6b7280' }}>{new Date(a.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}</td>
                  <td className="px-5 py-3.5">
              ...
              {apps.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-12 text-center text-sm" style={{ color: '#9ca3af' }}>No bookings yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="rounded-2xl overflow-hidden bg-white" style={{ border: '1px solid #eef2f7' }}>
        <div className="px-6 py-4 flex items-center justify-between border-b" style={{ borderColor: '#eef2f7' }}>
          ...
          <Link href="/admin/inquiries" className="text-xs font-bold px-4 py-2 rounded-lg" style={{ background: 'linear-gradient(to right,#3CA5D4,#0E3254)', color: '#fff' }}>View all →</Link>
        </div>
        <div className="divide-y" style={{ borderColor: '#f3f4f6' }}>
          {(recentInquiries ?? []).map((inq) => (
            <div key={inq.id} className="px-6 py-4 flex items-start gap-4 hover:bg-[#f8fbff] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                  ...
                  <span className="text-xs" style={{ color: '#9ca3af' }}>{inq.email}</span>
                </div>
          ...
          {(recentInquiries ?? []).length === 0 && (
            <div className="px-6 py-10 text-center text-sm" style={{ color: '#9ca3af' }}>No inquiries yet.</div>
          )}
        </div>
      </div>
    </div>
