  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: '#9ca3af' }}>Manage site configuration, email delivery, and payments</p>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 p-1 rounded-2xl" style={{ background: '#f8fbff', border: '1px solid #eef2f7' }}>
        {TABS.map((tab) => (
          <button
            key={tab.id}
            ...
            style={{
              background: activeTab === tab.id ? 'linear-gradient(to right,#3CA5D4,#0E3254)' : 'transparent',
              color: activeTab === tab.id ? '#ffffff' : '#6b7280',
              boxShadow: activeTab === tab.id ? '0 2px 12px rgba(60,165,212,0.25)' : 'none',
            }}
          >
            {tab.icon}
      ...
      {/* Tab panels */}
      <div className="rounded-2xl overflow-hidden bg-white" style={{ border: '1px solid #eef2f7' }}>
        ...
        {activeTab === 'email' && (
          <div className="p-6 space-y-5">
            <div className="rounded-xl p-4 text-sm" style={{ background: '#f0f9ff', color: '#0369a1', border: '1px solid #bae6fd' }}>
              <strong>Gmail:</strong> Use your Gmail address as username, and an App Password (not your account password). Enable 2FA first, then create an App Password at myaccount.google.com/apppasswords.
            </div>
            <Row label="SMTP Host">
            ...
            <Row label="App Password">
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowSmtpPass(!showSmtpPass)} className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center w-6 h-6 rounded-md hover:bg-gray-100 transition-colors" style={{ color: '#9ca3af' }}>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: '2px solid #eef2f7' }}>
              <div className="px-5 py-4 flex items-center justify-between" style={{ background: '#f8fbff', borderBottom: '1px solid #eef2f7' }}>
                <div>
                  <div className="font-bold text-sm" style={{ color: '#0A385A' }}>Stripe Mode</div>
                  <div className="text-xs mt-0.5" style={{ color: '#9ca3af' }}>Controls which keys are used for all payments</div>
                </div>
                {/* Toggle pill */}
                <div className="flex items-center rounded-xl p-1 gap-1" style={{ background: '#eef2f7' }}>
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: settings.stripe_mode === 'test' ? '0 2px 8px rgba(245,158,11,0.35)' : 'none',
                    }}
                  >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: settings.stripe_mode === 'live' ? '0 2px 8px rgba(16,185,129,0.35)' : 'none',
                    }}
                  >
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5">
              ...
              <div
                ...
                style={{
                  background: settings.stripe_mode === 'live' ? 'rgba(16,185,129,0.08)' : 'rgba(245,158,11,0.08)',
                  color: settings.stripe_mode === 'live' ? '#059669' : '#d97706',
                }}
              >
                <span className="w-2 h-2 rounded-full animate-pulse flex-shrink-0" style={{ background: settings.stripe_mode === 'live' ? '#10b981' : '#f59e0b' }} />
            ...
            {settings.stripe_mode === 'live' && (
              <div className="rounded-xl p-4 flex items-start gap-3 text-sm" style={{ background: '#fff1f2', color: '#be123c', border: '1px solid #fecdd3' }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-5 h-5 flex-shrink-0 mt-0.5"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                <div>
                  <strong>Live mode is active.</strong> Customers will be charged real money. Make sure your live keys are correct before saving.
                </div>
              </div>
            )}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#f59e0b' }}>Test Keys</span>
              ...
              <div className="rounded-xl p-4 text-xs" style={{ background: '#fffbeb', color: '#92400e', border: '1px solid #fde68a' }}>
                Use <strong>pk_test_...</strong> and <strong>sk_test_...</strong> keys from your Stripe dashboard → Developers → API keys.
              </div>
              <Row label="Test Publishable Key (pk_test_...)">
                <Input value={settings.stripe_test_publishable_key} onChange={set('stripe_test_publishable_key')} placeholder="pk_test_51..." />
              </Row>
              <Row label="Test Secret Key (sk_test_...)">
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowTestSecret(!showTestSecret)} className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center w-6 h-6 rounded-md hover:bg-gray-100 transition-colors" style={{ color: '#9ca3af' }}>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#10b981' }}>Live Keys</span>
                {settings.stripe_mode === 'live' && (
              ...
              <div className="rounded-xl p-4 text-xs" style={{ background: '#f0fdf4', color: '#14532d', border: '1px solid #bbf7d0' }}>
                Use <strong>pk_live_...</strong> and <strong>sk_live_...</strong> keys from your Stripe dashboard → Developers → API keys.
              </div>
              <Row label="Live Publishable Key (pk_live_...)">
                <Input value={settings.stripe_live_publishable_key} onChange={set('stripe_live_publishable_key')} placeholder="pk_live_51..." />
              </Row>
              <Row label="Live Secret Key (sk_live_...)">
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowLiveSecret(!showLiveSecret)} className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center w-6 h-6 rounded-md hover:bg-gray-100 transition-colors" style={{ color: '#9ca3af' }}>
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          className="flex items-center gap-2 px-8 py-3.5 rounded-xl font-bold text-sm text-white disabled:opacity-60"
...
function Input({ value, onChange, placeholder, type = 'text' }: {
  ...
  return (
    <input type={type} value={value} onChange={onChange} placeholder={placeholder}
      className="rounded-xl border px-3.5 py-2.5 text-sm focus:outline-none focus:border-[#3CA5D4] w-full"
